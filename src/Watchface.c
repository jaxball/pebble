...
static void init_clock(Window *window) {
  ...
  time_layer = text_layer_create(GRect(0, 35, bounds.size.w, bounds.size.h-100));
  text_layer_set_text_alignment(time_layer, GTextAlignmentCenter);
  text_layer_set_text_color(time_layer, GColorWhite);
  text_layer_set_background_color(time_layer, GColorClear);
  s_time_font = fonts_load_custom_font(resource_get_handle(RESOURCE_ID_FONT_ROBOTO_REG_40));
...
static void init_location_search(Window *window) {
  ...
  location_layer = text_layer_create((GRect) { .origin = { 0, 15 }, .size = { bounds.size.w, 100 } });
  ...
  text_layer_set_text(text_layer, "Weather");
  ...
  label_layer = text_layer_create((GRect) { .origin = { 0, 110 }, .size = { bounds.size.w, 100 } });
  text_layer_set_text(label_layer, "Nearest Hotels:");
  ...
  text_layer = text_layer_create((GRect) { .origin = { 0, 125 }, .size = { bounds.size.w, bounds.size.h } });
  ...
  // Tuplet initial_location[] = {
  //    TupletCString(CITY, "Location")
  // };
